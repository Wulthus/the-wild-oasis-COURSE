...
export default function SalesChart({ bookings, numDays }){
  ...
  const data = allDates.map((date)=>{
    return {
      ...
      totalSales: bookings.filter((booking=>isSameDay(date, new Date(booking.created_at)))).reduce((acc, cur)=>acc+ cur.price_total, 0),
      extrasSales: bookings.filter((booking=>isSameDay(date, new Date(booking.created_at)))).reduce((acc, cur)=>acc+ cur.price_extras, 0),
    }
  })
  ...
  return (
    <StyledSalesChart>
      <Heading as='h2'>Sales: {format(allDates.at(0), "MMM dd yyyy")} &mdash; {format(allDates.at(-1), "MMM dd yyyy")}</Heading>
      <ResponsiveContainer height={300} width='100%'>
        <AreaChart data={data}>
          <XAxis dataKey="label" tick={{fill: colors.text}} tickLine={{stroke: colors.text}}></XAxis>
          <YAxis unit="$" tick={{fill: colors.text}} tickLine={{stroke: colors.text}}></YAxis>
